...
Embedded systems can reboot/rest randomly but AQI requires a 24 hour avergage. To work around this problem, this class allows you to define  a one second sample aggregated to a one minute average, cascaded to a one hour average, cascaded to a one day average (and beyond if necessary), writing the results to storage and reloading as necessary between resets. 
